...
;(async () => {
  ...
  for (let i = now; i <= now + CALCULATE_FUTURE_THERESHOLD; i += 86400000) {
    ...
    if (written.result[0] === vTuberExpected) {
      console.log(
        `You are expected to have ${vTuberExpected} at ${
          getRandomSeedBasedOnDate(new Date(i)) + 100
        }.`
      )
      return
    }
  ...
  console.log(
    `You are not expected to have ${vTuberExpected} in ${
      CALCULATE_FUTURE_THERESHOLD / (24 * 60 * 60 * 1000)
    } days.`
  )
})()
